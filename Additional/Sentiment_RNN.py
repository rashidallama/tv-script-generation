...
vocab_to_int = {vocab[i-1]: i for i in range(1,len(vocab)+1)}

# Convert the reviews to integers, same shape as reviews list, but with integers
reviews_ints = [[vocab_to_int[x] for x in y.split()] for y in reviews]
...
labels = np.array([labels[i] for i in range(len(reviews_ints)) if len(reviews_ints[i])>0])
...
features = []
for ri in reviews_ints:
    if len(ri)>seq_len:
        features.append(ri[0:seq_len])
    else:
        len_ri = len(ri)
        features.append([0]*(seq_len-len_ri)+ri)
...
with graph.as_default():
    ...
    labels_ = tf.placeholder(tf.int32, shape= [None, None], name='outputs')
